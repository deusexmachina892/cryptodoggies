...
contract DetailedERC721 is ERC721{
    ...
    function symbol() public view returns (string _sym);
...
contract CryptoDoggies is AccessControl, DetailedERC721{
    ...
    function generateRandomDna() private view returns(bytes5 _dna){
        uint256 lastBlockNumber = block.number - 1;
        bytes32 hashVal = bytes32(blockhash(lastBlockNumber));
        _dna = bytes5((hashVal & 0xffffffff) << 216);
    ...
    function getAllTokens() public view returns(uint256[], uint256[], address[]){
    ...
    function tokensOf(address _owner) public view returns (uint256[]){
        ...
        if(tokenCount == 0){
            ...
            uint256[] memory _ownerTokens = new uint256[](tokenCount);
            uint256 total = totalSupply();
            uint256 tokenIndex = 0;

            for(uint256 i=0; i<total; i++){
                if(tokenIdToOwner[i] == _owner){
                    _ownerTokens[tokenIndex] = i;
                    tokenIndex++;
                }
            } 
        }
    ...
    function purchase(uint256 _tokenId) public payable whenNotPaused{
        ...
        //ceoAddress.transfer(contractCut);
    }
    ...
    function nextPriceOf(uint256 _tokenId) public view returns (uint256 _nextPrize){
    ...
    //to know whether the function is ERC721 compliant
